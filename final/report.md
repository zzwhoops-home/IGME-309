...
## Vibing Tree
![Image of audio visualizer](image.png)
...
- Not really removed, but the lake is just a square mesh that I made a lot bigger. So it looks more like an ocean I've plopped in the middle of the screen and I couldn't get depth perception to work that well so it looks kinda weird? But I'll take it.

- I've axed the sun, which unfortunately makes it less synthwave-y but I'm definitely out of time and energy.

Overall, I think it is a design that runs well and matches my design document. There's nothing that's changed significantly or made much simpler; I actually took the time to try and implement a tree that could be randomly generated by pressing 1.
...
When drawing, I learned from AI that you can draw a wireframe by offsetting the polygons drawn so they don't overlap as much with `
	glEnable(GL_POLYGON_OFFSET_FILL);
	glPolygonOffset(1.0f, 1.0f);
`, but also draw a wireframe with `
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);`.
    This I think sells the synthwave vibe I was going for more, although the wintry colors make it less so.

### Backgrounds

Changing the background is done in main with a couple of float[4] variables holding rgba colors, and a bunch of lerp functions. The function `updateBGColor` does the job. I use the `AudioAnalyzer`'s `duration()` and `getCurrentTime()` functions to calculate a `ratio`, which then lerps between the colors of (synthwave) twilight, sunset/rise, and daytime.
...
I found the function `glPushAttrib(GL_ALL_ATTRIB_BITS);` and its pop counterpart to be incredibly useful for many situations where I wanted to change a couple of attributes but didn't want to have to reset them back.