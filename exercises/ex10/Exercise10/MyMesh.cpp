...
{
	// initialize max/min verts
	minVert.x = maxVert.x = vertices[0];
	...
	for (int i = 1; i < vertNum; i++) {
		...
		maxVert.x = std::max(maxVert.x, x);
		maxVert.y = std::max(maxVert.y, y);
		maxVert.z = std::max(maxVert.z, z);

		minVert.x = std::min(minVert.x, x);
		minVert.y = std::min(minVert.y, y);
		minVert.z = std::min(minVert.z, z);
	}
}
...
void MyMesh::drawAABB()
{
	...
	float l = maxVert.x - minVert.x;
	float h = maxVert.y - minVert.y;
	float w = maxVert.z - minVert.z;
